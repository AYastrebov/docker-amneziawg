...
- The container never writes `MTU`; `awg-quick` derives `route MTU − 80` = 1420, which ignores `S4` and fragments full-size packets when `S4 > 20` (IPv4) or on IPv6 endpoints. `ContentPaddingAddition`/transport `RandomTrailers` are capped to MTU / observed UDP window and cannot fragment. README "MTU" is the user-facing guidance (1280 safe default).
