...
- The container never writes `MTU`; `awg-quick` derives `route MTU − 80` = 1420, which ignores `S4` and fragments full-size packets when `S4 > 20` (IPv4) or on IPv6 endpoints. `ContentPaddingAddition`/transport `RandomTrailers` are capped at the observed UDP window (NOT the MTU — the window counts received datagrams too, so full-size sends grow slightly) and do not fragment transport packets; kernel modules before v3.1.20260906 appended trailers to I1-I5/junk packets too, producing rare oversized handshake-burst packets (root-caused, fixed upstream in `4569c4c6`; see `docs/awg-performance.md`). README "MTU" is the user-facing guidance: 1280 for ordinary paths, `path − 60/80 − S4` when the path itself is constrained (a true 1280-byte path needs 1208/1188, not 1280).
