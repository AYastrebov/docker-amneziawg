...
### Added
- IPv6 inside the tunnel: peers get a ULA address by default (derived from `INTERNAL_SUBNET`), closing the dual-stack leak. `IP6_SUBNET` sets or disables (`off`) the prefix; `IP6_EXIT` (`auto|nat|routed|off`) controls egress. With `enable_ipv6: true` on the Docker network (shipped commented out in `docker-compose.yml`, needs Docker >= 27), IPv6 is masqueraded like IPv4; without it, IPv6 is rejected fast and CoreDNS returns no AAAA records. `auto` never picks a mode whose `PostUp` would fail: a container with no IPv6 stack gets no IPv6 address at all, and a host without a usable `ip6table_nat` falls back to `off` with the reason logged. Existing installations must add `import /config/coredns/generated/*.conf` to their `/config/coredns/Corefile` by hand for AAAA suppression - see README "Upgrading".
