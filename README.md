...
Be precise about what the "IPv6 minimum" argument guarantees, though: the 1280-byte floor applies to the packet **on the wire**, and a tunnel MTU of 1280 produces wire packets of `1340 + S4` bytes. On a path whose own MTU really is 1280 — DS-Lite, some LTE and tunnel-in-tunnel setups — those still fragment. The truly-safe-everywhere tunnel MTU is `1280 − 60 − S4` (1208 at `S4 = 12`), which keeps the outer packet at or under 1280. We verified this on such a path: at tunnel MTU 1208 every full-size datagram measured exactly 1280 on the wire, and 1280 would have fragmented all of them. Measure your path (`ping -M do` binary search) rather than assuming; use 1280 when the path is normal or unknown-but-probably-normal, and `path − 60 − S4` when you know the path is constrained.
...
| Path that is itself constrained to ~1280 (DS-Lite, tunnel-in-tunnel, some LTE) | `path − 60 − S4` | The outer packet must fit the *path*, not the IPv6 floor: at `S4 = 12` a 1280-byte path needs tunnel MTU **1208**. Measure with a `ping -M do` binary search |
...
Do not go above the derived number expecting more speed: the tunnel MTU is a ceiling, and every byte above the path limit is paid back as fragmentation. And do not go below `path − 60 − S4` either — that only adds per-packet overhead. On a normal 1500-byte path that floor argument lands at 1280; on a constrained path the right value is lower (1208 on a true 1280-byte path at `S4 = 12`), and it is derived, not a magic number.
