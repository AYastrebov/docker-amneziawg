...
Do not go above the derived number expecting more speed: the tunnel MTU is a ceiling, and every byte above the path limit is paid back as fragmentation. And do not go below `path − 60 − S4` (IPv4) / `path − 80 − S4` (IPv6) either — that only adds per-packet overhead. On a normal 1500-byte path that argument lands around 1280; on a constrained path the right value is lower (1208 IPv4 / 1188 IPv6 on a true 1280-byte path at `S4 = 12`), and it is derived, not a magic number.
