...
| Attribute | Value |
|---|---|
| Base | `ghcr.io/linuxserver/baseimage-alpine:3.21` |
...
| Attribute | Value |
|---|---|
| Base | `alpine:3.21` (no LSIO overhead — the API is a single static Go binary + `awg` CLI) |
| Contents | `awg` binary, `awg-api` binary, lightweight entrypoint |
| Size | ~25–30 MB total |
| Port | `8081/tcp` |

**Why `alpine:3.21` and not `ghcr.io/linuxserver/baseimage-alpine`?**  
...
|---|---|---|
| `docker-amneziawg` | Tracks upstream `amneziawg-tools` version | `latest`, `v1.0.20260223`, semver |
...
| API image base: `alpine:3.21` or LSIO? | **`alpine:3.21`** | API doesn't need s6-overlay or init hooks. |
